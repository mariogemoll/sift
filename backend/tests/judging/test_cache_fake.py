...
def test_batch_contract_and_serialization(profile: Profile) -> None:
    asker: Asker = FakeAsker()
    asks = [ask_for(Document(str(i), f"Paper {i}"), profile) for i in range(4)]
    results = asker(asks)
    assert len(results) == len(asks)
    assert list(asker(list(reversed(asks)))) == list(reversed(results))
    assert asker([]) == []
...
def test_all_question_types() -> None:
    ask = Ask(
        {"document": "text"},
        {
            "n": Noul("yes?"),
...
def test_reweighting_reuses_judgments_without_inference(profile: Profile) -> None:
    document = Document("id", "A robotics paper")
    ask = ask_for(document, profile)
    stored = {cache.key_for(ask.state, ask.asked): FakeAsker()([ask])[0]}
    reweighted = replace(
        profile,
        merit_weight=0.95,
    ...
    reused = stored[cache.key_for(next_ask.state, next_ask.asked)]
    assert (
...
def test_cache_covers_all_inputs(profile: Profile) -> None:
    ...
    original = cache.key_for(ask.state, ask.asked)
    assert original == cache.key_for(
        dict(reversed(list(ask.state.items()))), dict(reversed(list(ask.asked.items())))
    )
    for changed in [
        ...
        assert cache.key_for(changed.state, changed.asked) != original
    assert cache.key_for({}, {"q": Noul("same")}) != cache.key_for(
        {}, {"q": Score("same", ("a", "b"))}
    )


def test_cache_covers_question_details() -> None:
    base = cache.key_for({}, {"q": Score("same", ("low", "high"))})
    assert base != cache.key_for({}, {"q": Score("same", ("high", "low"))})
    assert base != cache.key_for({}, {"renamed": Score("same", ("low", "high"))})
    assert cache.key_for({}, {"q": Choice("same", {"a": "A", "b": "B"})}) != cache.key_for(
        {}, {"q": Choice("same", {"a": "changed", "b": "B"})}
    )
